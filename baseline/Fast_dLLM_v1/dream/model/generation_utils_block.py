...
def _update_block_completion_states(block_states, decoded_token_threshold):
    """
    ...
    for block_id in sorted(block_states.keys()):
        ...
        decoded_tokens = (
            block_states[block_id]["total_masks"]
            - block_states[block_id]["mask_count"]
        )
...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        # # print(f"[DEBUG] multi_block={multi_block}, block_add_threshold={block_add_threshold}, decoded_token_threshold={decoded_token_threshold}")
    ...
    def _sample(
        ...
        # init values
        # # print(f"\033[91m[DEBUG-MB] Originial Sample \033[0m")
        ...
        nfe = 0  # Track number of function evaluations
        ...
        # In block-based generation, steps is per-block steps, not total steps
        ...
        for num_block in range(num_blocks):
            ...
            while True:
                ...
                if alg == 'confidence_threshold':
                    ...
                    if dual_cache:
                        x_ = torch.zeros_like(x[:, current_block_start:current_block_end], device=self.device, dtype=torch.long) + mask_token_id
                        full_entropy = torch.full_like(x[:, current_block_start:current_block_end], torch.inf, device=self.device, dtype=logits.dtype)
                    else:
                        x_ = torch.zeros_like(x[:, current_block_start:], device=self.device, dtype=torch.long) + mask_token_id
                        full_entropy = torch.full_like(x[:, current_block_start:], torch.inf, device=self.device, dtype=logits.dtype)
                    
                    x_[mask_index] = x0.clone()
                    full_entropy[mask_index] = entropy
                    full_entropy[:, block_length:] = torch.inf
