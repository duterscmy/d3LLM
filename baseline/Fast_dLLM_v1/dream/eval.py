...
import types
# DreamModel and DreamConfig will be imported dynamically based on multi_block setting
import time
import os
import json
...
class Dream(LM):
    def __init__(
        ...
        multi_block: Optional[bool] = False,
        block_add_threshold: Optional[float] = 0.5,
        decoded_token_threshold: Optional[float] = 0.5,
        cache_warmup_steps: Optional[int] = 2,
        block_length: Optional[int] = 32,
        cache_delay_iter: Optional[int] = 10000,
        ...
        **kwargs,
        ...
        # Initialize generation parameters BEFORE creating model
        ...
        # print(f"[DEBUG-PARAMS] Initialized: use_cache={use_cache}, multi_block={multi_block}, alg={alg}")
        ...
        self.show_speed = True  # Enable speed statistics
        self.is_instruct = True if 'instruct' in pretrained.lower() or "d3llm" in pretrained.lower() or 'fast_dllm_v2' in pretrained.lower() else False
    @property
    def batch_size(self):
    ...
    def _create_model_and_tokenizer(self, pretrained, dtype, trust_remote_code):
        # Import DreamModel and bind methods based on multi_block setting
        if self.multi_block:
            # Use utils_Dream for multi-block generation
            import sys
            ...
            # Use Fast_dLLM_v1 for standard generation
        ...
        if self.multi_block:
            self.model.generate_multi_block = types.MethodType(DreamGenerationMixin.generate_multi_block, self.model)
            self.model._sample_multi_block = types.MethodType(DreamGenerationMixin._sample_multi_block, self.model)
            self.model._sample_multi_block_kv_cache = types.MethodType(DreamGenerationMixin._sample_multi_block_kv_cache, self.model)
            self.model._prepare_inputs = types.MethodType(DreamGenerationMixin._prepare_inputs, self.model)
        elif self.use_cache:
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        if self.is_instruct:
            m = [{"role": "user", "content": prompts[0]}]
            user_input = self.tokenizer.apply_chat_template(m, add_generation_prompt=True, tokenize=False)
            prompt_ids = self.tokenizer(user_input, return_tensors="pt")['input_ids']
        else:
        ...
        if self.multi_block:
            # Use generate_multi_block from d3llm_DREAM
            generation_ids, nfe = self.model.generate_multi_block(
                prompt_ids,
            ...
            self.diffusion_steps = nfe  # Update with actual NFE
        else:
        ...
        # decode
        # print(f"[DEBUG-EVAL] generation_ids.sequences.shape={generation_ids.sequences.shape}")
        # print(f"[DEBUG-EVAL] prompt_ids.shape={prompt_ids.shape}, prompt_ids[0] len={len(prompt_ids[0])}")
        # print(f"[DEBUG-EVAL] generation_ids.sequences[0] len={len(generation_ids.sequences[0])}")
        # print(f"[DEBUG-EVAL] Generated part (first 20 tokens): {generation_ids.sequences[0, len(prompt_ids[0]):len(prompt_ids[0])+20].tolist()}")
        
        responses = [
            self.tokenizer.decode(g[len(p) :].tolist()).split(self.tokenizer.eos_token)[0]
            for p, g in zip(prompt_ids, generation_ids.sequences)
        ]
        # print(f"[DEBUG-EVAL] Response: {responses[0][:100]}")  # First 100 chars
        # Count tokens by re-encoding the response
        num_tokens = len(self.tokenizer(responses[0])["input_ids"])
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        res = []
        # Re-bind generation methods (in case of torch.compile)
        if self.multi_block:
            import sys
            sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))
            from d3llm.d3llm_DREAM.d3llm_dream_generate_util import DreamGenerationMixin
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            responses, tokens_in_batch = self._generate_batch(contexts)
            end_time = time.time()
            ...
            if batch_count >= warmup_steps:
                ...
                if self.show_speed:
                    ...
                    num_nfe += self.diffusion_steps
            
            # Only rank 0 prints per-sample details
            if self.rank == 0:
                print('=' * 20)
                print('question: ', contexts[0][:min(len(contexts[0]), 100)])
                print('answer: ', responses[0][:min(len(responses[0]), 100)])
                print('=' * 20, end='\n\n')
                
                print(f"NFE (Number of Function Evaluations): {self.diffusion_steps}")
                
                if batch_count >= warmup_steps:
                    print(f"Total time taken (after warmup): {run_time:.2f} seconds")
                    print(f"Total NFE is {num_nfe}")
                    if run_time > 0 and num_nfe > 0:
                        tokens_per_second = num_tokens / run_time
                        print(f"Throughput: {tokens_per_second:.2f} tokens/s")
                        print(f"Total tokens generated: {num_tokens}")
                        print(f"Token per step: {num_tokens / num_nfe:.2f}")
        ...
        if self.show_speed and hasattr(self, 'accelerator'):
            # Gather statistics from all ranks
            num_tokens_tensor = torch.tensor([num_tokens], device=self.device, dtype=torch.float32)
            num_nfe_tensor = torch.tensor([num_nfe], device=self.device, dtype=torch.float32)
            run_time_tensor = torch.tensor([run_time], device=self.device, dtype=torch.float32)
            
            torch.distributed.all_reduce(num_tokens_tensor, op=torch.distributed.ReduceOp.SUM)
            torch.distributed.all_reduce(num_nfe_tensor, op=torch.distributed.ReduceOp.SUM)
            torch.distributed.all_reduce(run_time_tensor, op=torch.distributed.ReduceOp.SUM)
            
            if self.rank == 0:
                total_tokens = int(num_tokens_tensor.item())
                total_nfe = int(num_nfe_tensor.item())
                total_time = run_time_tensor.item()  # Calculate average manually
                
                print(f"\n{'='*60}")
                print(f"Aggregated Statistics Across {self.world_size} Ranks:")
                print(f"  Total tokens generated: {total_tokens}")
                print(f"  Total NFE: {total_nfe}")
                print(f"  Average time taken: {total_time:.2f} seconds")
                if total_time > 0 and total_nfe > 0:
                    print(f"  Overall throughput: {total_tokens / total_time:.2f} tokens/s")
                    print(f"  Overall token per step: {total_tokens / total_nfe:.2f}")
                print(f"{'='*60}\n")
