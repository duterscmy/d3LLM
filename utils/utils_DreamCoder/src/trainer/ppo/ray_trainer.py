...
from src.trainer.ppo.metric_utils import (
...
class RayPPOTrainer(object):
    ...
    def __init__(
        ...
        if "wandb" in self.config.trainer.logger:
            self.wandb_table = self._create_wandb_table()
    ...
    def _create_wandb_table(self):
        """创建wandb表格用于记录生成结果"""
        columns = ["step", "uid", "response_idx", "reward", "response_text"]
        return wandb.Table(columns=columns)

    def _update_wandb_table(self, step, uids, responses, rewards):
        """更新wandb表格"""
        if self.wandb_table is not None:
            for i, (uid, response, reward) in enumerate(zip(uids, responses, rewards)):
                self.wandb_table.add_data(step, uid, i, reward, response)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    batch = batch.repeat(
                        ...
                        * self.config.actor_rollout_ref.actor.mask_epochs,
                        interleave=True,
                    )
                    ...
                    if self.config.trainer.balance_batch:
                        ...
                        ), "ppo_epochs must be 1 to use balance_batch"
                    ...
                    with _timer("adv", timing_raw):
                        # deduplicate batch (duplicated by ppo_epochs)
                        reward_batch = dict(
                            batch.batch.reshape(
                                -1,
                                self.config.actor_rollout_ref.actor.ppo_epochs
                        ...
                        reward_batch = DataProto.from_dict(
                            reward_batch, reward_non_tensor_batch
                        )
                        ...
                        if self.use_rm:
                            ...
                            reward_tensor = self.rm_wg.compute_rm_score(reward_batch)
                            reward_batch = reward_batch.union(reward_tensor)

                        # we combine with rule-based rm
                        reward_tensor = self.reward_fn(reward_batch)
                        reward_batch.batch["token_level_scores"] = reward_tensor

                        # log generations to wandb table
                        if self.wandb_table is not None:
                        ...
                        # compute solve rate
                        uids = reward_batch.non_tensor_batch["uid"]
                        ...
                        # Log to metrics
                        metrics["batch/solve_mean"] = reward_tensor.sum(-1).mean().item()
                        metrics["batch/solve_none"] = solve_none / len(unique_uids)
                        metrics["batch/solve_all"] = solve_all / len(unique_uids)
                        metrics["batch/solve_partial"] = (
                            len(unique_uids) - solve_none - solve_all
                        ) / len(unique_uids)

                        if self.config.actor_rollout_ref.actor.get(
                            "dynamic_filtering", False
                        ):
                            # select those uid that solved partial
                            valid_uids = np.unique(uids[valid_mask])
                            invalid_uids = np.unique(uids[~valid_mask])
                            sampled_valid_uids = np.random.choice(
                                valid_uids, size=len(invalid_uids), replace=True
                            )

                            for invalid_uid, sampled_valid_uid in zip(
                                invalid_uids, sampled_valid_uids
                            ):
                                invalid_uid_mask = uids == invalid_uid
                                sampled_valid_uid_mask = uids == sampled_valid_uid

                                # replace invalid with valid
                                new_uid = str(uuid.uuid4())

                                for k, v in reward_batch.batch.items():
                                    v[invalid_uid_mask] = v[sampled_valid_uid_mask]

                                for k, v in reward_batch.non_tensor_batch.items():
                                    v[invalid_uid_mask] = v[sampled_valid_uid_mask]

                                for k, v in batch.batch.items():
                                    v[
                                        invalid_uid_mask.repeat(
                                            self.config.actor_rollout_ref.actor.ppo_epochs
                                            * self.config.actor_rollout_ref.actor.mask_epochs
                                        )
                                    ] = v[
                                        sampled_valid_uid_mask.repeat(
                                            self.config.actor_rollout_ref.actor.ppo_epochs
                                            * self.config.actor_rollout_ref.actor.mask_epochs
                                        )
                                    ]
                        ...
                        if not self.config.actor_rollout_ref.actor.get(
                            ...
                            reward_batch, kl_metrics = apply_kl_penalty(
                                reward_batch,
                            ...
                            reward_batch.batch["token_level_rewards"] = (
                                reward_batch.batch["token_level_scores"]
                            )

                        # compute advantages, executed on the driver process
                        reward_batch = compute_advantage(
                            reward_batch,
                        ...
                        reward_batch = reward_batch.repeat(
                            self.config.actor_rollout_ref.actor.ppo_epochs
                            * self.config.actor_rollout_ref.actor.mask_epochs,
                            interleave=True,
                        )
                        batch.batch["token_level_rewards"] = reward_batch.batch[
                            "token_level_rewards"
                        ]
                        batch.batch["token_level_scores"] = reward_batch.batch[
                            "token_level_scores"
                        ]
                        batch.batch["advantages"] = reward_batch.batch["advantages"]
                        batch.batch["returns"] = reward_batch.batch["returns"]
                ...
                # collect metrics
                batch.meta_info = batch.meta_info | {
                    "eos_token_id": self.tokenizer.eos_token_id,
                }  # for compute response length
                metrics.update(
                ...
                # 每个epoch结束时保存wandb表格
                if self.wandb_table is not None:
                    logger.logger["wandb"].log(
                        {"generations_table": self.wandb_table}, step=self.global_steps
                    )
