...
def check_accuracy(
    raw_pred: str, gt: str, any_order: bool = False
) -> Tuple[bool, bool]:
    """
...
def compute_score(
    model_output: str, ground_truth: str, extra_info: any = None
) -> Dict[str, bool]:
    """
    Compute score dict for evaluation harness.
    """
    correct, no_answer = check_accuracy(
        str(model_output), str(ground_truth), any_order=False
    )
    return correct, {"is_valid": not no_answer, "model_output": model_output}
