        ...
        for num_block in range(num_blocks):
            ...
            while True:  
                ...
                if alg == 'entropy_threshold':
                    ...
                    # Ensure topk_k doesn't exceed the size of the tensor
                    topk_k = min(current_transfer_tokens.item(), full_entropy.shape[1])
                    selected_entropy, select_index = torch.topk(full_entropy, topk_k, dim=1, largest=False)
                    ...
                    if topk_k > 0:
                        ...
                        for idx in range(1, topk_k):
                            if selected_entropy[0, idx] < threshold:
                                transfer_index[0, select_index[0, idx]] = True
                            else:
                                transfer_index[0, select_index[0, idx]] = False
