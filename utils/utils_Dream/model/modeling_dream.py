...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        # Handle KV cache with update_kvcache support
        if past_key_value is not None:
            if update_kvcache == 0:
        ...
        # Slice attention mask to match key_states length (same as D2F_eval)
        if attention_mask is not None and isinstance(attention_mask, torch.Tensor):
            attention_mask = attention_mask[:, :, :, : key_states.shape[-2]]

        # SDPA with memory-efficient backend is currently (torch==2.1.2) bugged with non-contiguous inputs with custom attn_mask,
        # Reference: https://github.com/pytorch/pytorch/issues/112577.
        if query_states.device.type == "cuda" and isinstance(attention_mask, torch.Tensor):
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        # Initialize DynamicCache if using cache
